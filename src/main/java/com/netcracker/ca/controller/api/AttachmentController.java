...
public class AttachmentController extends BaseApiController {
	...
	@GetMapping({"admin/api/file/{fileId}", "student/api/file/{fileId}", "curator/api/file/{fileId}"})
	...
	@DeleteMapping({"admin/api/file/{fileId}", "curator/api/file/{fileId}"})
	public void delete(@PathVariable int fileId) {
		attService.delete(fileId);
	}

	
	@GetMapping({"curator/api/team/{teamId}/files", "student/api/team/{teamId}/files"})
	public List<Attachment> getTeamAttachments(HttpSession session) {
		Team team = (Team) session.getAttribute("team");
		return attService.getTeamAttachments(team.getId());
	}
	
	@PostMapping("curator/api/team/{teamId}/file")
	public Attachment uploadForTeam(@RequestParam MultipartFile file, @RequestParam("text") String text, HttpSession session)
			throws IOException {
		Team team = (Team) session.getAttribute("team");
		//add validation for file (not empty, <maxSize)
		try (InputStream input = file.getInputStream()) {
			Attachment att = attFactory.build(file.getName(), text, file.getContentType(), team.getId(), true);
			return attService.addToTeam(att,input, team.getId());
		}
	}
