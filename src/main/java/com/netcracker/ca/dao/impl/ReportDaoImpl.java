...
public class ReportDaoImpl implements ReportDao {
    ...
    private static String SQL_PROJECT_REPORT = "SELECT projects.title, involved.count AS involv, expelled.count AS expell, " +
            ...
            "INNER JOIN\n" +
            "(SELECT projects.id, COUNT (feedback.id)\n" +
            "FROM projects INNER JOIN feedback ON projects.id = feedback.project_id\n" +
            "INNER JOIN student_for_hr_status_types ON student_for_hr_status_types.id = feedback.status_for_hr_id\n" +
    ...
    private static String SQL_STUDENT_REPORT = "SELECT users.first_name, users.last_name, feedback.protection_report, feedbacks.general_report, feedbacks.tech_report,\n" +
    ...
    private static class StudentReportMapper implements RowMapper<StudentReport>{
        ...
        public StudentReport mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            sr.setProtection(rs.getString("protection_report"));
